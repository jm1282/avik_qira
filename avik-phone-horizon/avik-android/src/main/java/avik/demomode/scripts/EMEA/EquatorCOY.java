...
public class EquatorCOY {
    ...
    public void captureScreens() throws Exception {
        ...
        batteryPage.nextExperienceCard();
        takeScreenshot("DemoModeExperiences_EquatorCOY_EMEA_Battery_WirelessCharging");

        SpecsPage specsPage = batteryPage.nextExperience(SpecsPage.class);
        specsPage.captureSpecs(avikHandler, "EquatorCOY_EMEA", TOTAL_SCROLLS_TECHSPECS);

        DisclaimerPage disclaimerPage = specsPage.openDisclaimers();
        disclaimerPage.captureDisclaimers(avikHandler, "EquatorCOY_EMEA", TOTAL_SCROLLS_DISCLAIMERS);
    }
