 ...
 */
@AvikWinScript
public class TestExplorerPage {

    private static final String SCREENSHOT_PREFIX = "LenovoQiraPC_ExplorerPage_";
    private static final int HERO_CARD_COUNT = 5;
    ...
    public void main() throws Exception {
        ...
        captureHeroCards(explorerPage);

        explorerPage.tapChatHistoryCard();
        capture("chat_history");
        explorerPage.tapReturnToBasePage();
    }

    private void captureHeroCards(ExplorerPage explorerPage) throws Exception {
        long currentSignature = explorerPage.captureHeroSignature();

        for (int i = 1; i <= HERO_CARD_COUNT; i++) {
            capture("slide" + i);

            if (i < HERO_CARD_COUNT) {
                explorerPage.waitForHeroCardChange(currentSignature);
                currentSignature = explorerPage.captureHeroSignature();
            }
        }
    }
